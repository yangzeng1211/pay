...
import com.pay.config.SignConfig;
...
import com.pay.sign.HttpRequestWrapper;
import com.pay.sign.RSA;
import com.pay.sign.Request;
...
public abstract class Paymax extends PaymaxBase {
    ...
    protected static <T> T request(String url, String jsonReqData, Class<T> clazz) throws AuthorizationException, IOException, InvalidRequestException, InvalidResponseException {
        ...
        if(StringUtils.isBlank(SignConfig.SECRET_KEY)){
            throw new AuthorizationException("Secret key can not be blank.Please set your Secret key in com.pay.config.SignConfig");
        }

        if(StringUtils.isBlank(SignConfig.PRIVATE_KEY)){
            throw new AuthorizationException("RSA Private key can not be blank.Please set your RSA Private key  in com.pay.config.SignConfig");
        }

        if(StringUtils.isBlank(SignConfig.PAYMAX_PUBLIC_KEY)){
            throw new InvalidRequestException("Paymax Public key can not be blank.Please set your Paymax Public key in com.pay.config.SignConfig");
        }
